...
nfast = 12
nema = 9
macd, macdsignal, macdhist = talib.MACD(prices, fastperiod=nfast, slowperiod=nslow)
ema = talib.EMA(macd, nema)

if macd[-1] >= ema[-1] and macd[-2] < ema[-2]:
    print 'bullish crossover'
elif macd[-1] <= ema[-1] and macd[-2] > ema[-2]:
...
ax2.plot(r.date, ma10, color='blue', lw=2, label='MA (10)')
ax2.plot(r.date, ma50, color='red', lw=2, label='MA (50)')
